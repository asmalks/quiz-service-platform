...
BEGIN
  ...
  -- 3. Get created_by
  -- Ideally, quizzes created_by should be an auth.user. 
  -- For client admins without auth.users, they might need a placeholder or just use 0000...
  -- If there's a constraint, we will try to use a fallback or the admin_id if the constraint was dropped.
  -- Let's try to lookup auth.uid() if running in authenticated context, else use a placeholder UUID.
  ...
  -- Ensure that json_array_elements works and handles exceptions
